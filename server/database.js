...
exports.makeTransfer = function(uid,senderId,destId,satoshis,callback){
  assert(typeof senderId === 'number');
  assert(typeof destId === 'number');
  ...
  getUserFromId(senderId,function(err,sender){
    ...
    getClient(function(client, callback) {
      ...
      [satoshis,senderId],function(err,response){
        ...
        [satoshis,destId],function(err,response){
          ...
          if (response.rowCount !== 1)
            return callback(new Error('Unexpected dest balance row count: \n' + response));

          client.query("INSERT INTO transfers (id,sender_user_id,dest_user_id,amount) values($1,$2,$3,$4) ",[uid,senderId,destId,satoshis],function(err,response){
            if (err){
              console.log(err);
              return callback(err)
            }
